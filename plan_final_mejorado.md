...
- **1. Dimensión Medioambiental [E] (Environmental - 40%)**:
  ...
  - *Mesomunicipal (`gold_municipio_master`)*:
    - **Sobrecarga de Recursos e Hídrica:** Ratio de Población Turística Equivalente sobre Población Residente (`pob_turistica_equiv / poblacion_total` del ISTAC). Mide la presión per cápita directa sobre plantas desaladoras, redes de saneamiento, depuración de aguas residuales y recogida de residuos sólidos.
...
- **2. Dimensión Social [S] (Social - 40%)**:
  ...
  - *Mesomunicipal (`gold_municipio_empleo` & `gold_municipio_master`)*:
    - **Monocultivo y Vulnerabilidad Laboral:** `% dependencia hostelería` (`empleo_hosteleria / empleo_total * 100`). Los municipios con hiperconcentración hostelera (>45%) presentan una extrema vulnerabilidad social ante crisis exógenas (quiebras de turoperadores, pandemias).
    - **Tejido Emprendedor Endógeno:** `% autónomos` (`empleo_autonomos / empleo_total * 100`). Indicador de distribución de riqueza y microemprendimiento local frente a la dependencia exclusiva de salarios corporativos.
    - **Tensión Social y Empleo:** Evolución del desempleo (`paro_registrado`, `var_paro_pct`) y presión turística por habitante (`plazas_por_1000_hab`).
    - **Nivel de Renta:** `renta_bruta_irpf` per cápita municipal (ISTAC) y porcentaje de residentes extranjeros (`% poblacion_extranjera`).
...
- **3. Dimensión de Gobernanza y Regulación [G] (Governance - 20%)**:
  ...
  - *Mesomunicipal (`gold_municipio_master`)*:
    - **Formalización del Tejido Productivo:** `empresas_ss` por 1.000 habitantes (empresas con trabajadores asalariados dadas de alta en la Seguridad Social, ISTAC).
    - **Gobernanza de la Movilidad:** `parque_vehiculos_1000hab` (densidad del parque móvil y grado de congestión viaria insular).
