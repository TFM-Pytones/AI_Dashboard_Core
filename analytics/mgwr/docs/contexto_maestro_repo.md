...
**Última actualización:** 16 de septiembre de 2026 — Bloque 5 (Subtareas 5.1, 5.2 y 5.3) cerrado por completo. Ver nota al final de la sección 3.
**Actualización anterior:** 12 de septiembre de 2026 — actualización de la sección 3 (Gold) tras investigación real de variables del Bloque 5.
**Actualización anterior a esa:** 2 de septiembre de 2026
...
### Actualización 16-sep-2026 — Bloque 5 (5.1/5.2/5.3) cerrado, tablas Gold nuevas

- **`gold.gold_h3_ptna_v3`** — cerrada. Modelo MGWR/PTNA final, 2579 hexágonos, con `ptna_score` y `confianza_ptna` (3 criterios de confianza baja acumulados: periferia geográfica, clúster de colinealidad de altitud, y saturación de bandwidth — 259/2579 marcados como "baja" en total). Script: `analytics/mgwr/scripts/04_run_model.py` + `05_ptna_score.py` + `06_load_to_gold.py`.
- **`gold.gold_h3_esg_v1`** — cerrada, Índice ESG por hexágono. Ya estaba en producción desde antes de esta fecha, pero **sin script versionado en el repo** — se reconstruyó y formalizó en `07_gold_h3_esg.py`, validado fila por fila contra la tabla real (2579/2579 coinciden, diferencia máxima 0.0).
- **`gold.gold_bloque5_municipio_esg_v1`** — nueva. Índice ESG a nivel municipio (31/31 municipios, sin NULLs). El plan original (`plan_final_mejorado.md`, Subtarea 5.3) pedía 4 variables ISTAC mesomunicipales que nunca tuvieron ingesta real (`renta_bruta_irpf`, `poblacion_extranjera`, `empresas_ss`, `parque_vehiculos_1000hab`) — investigado exhaustivamente en bronze/silver/gold y **confirmado con el responsable de ISTAC del equipo que no van a estar disponibles**. Se reemplazaron por variables verificadas estadísticamente (VIF, cobertura, distribución) antes de usarlas. Script: `08_gold_municipio_esg.py`. Detalle completo de qué se descartó y por qué en `metodologia_bloque5_ptna.md`.
- **`gold.gold_bloque5_h3_oportunidad_v1`** — nueva. Cruce entre PTNA y ESG H3 (`ptna_score > 0` AND `esg_h3_score > 60`), el filtro combinado que el plan describía como objetivo final de interpretabilidad para TUI pero nunca se había construido. 247/2579 hexágonos marcados como "oportunidad ideal". Script: `09_gold_h3_oportunidad.py`.
- **`gold.gold_bloque5_municipio_anual_extra`** / **`gold.gold_bloque5_municipio_mensual_extra`** — nuevas, tablas satélite que resolvieron 2 de los 6 gaps mesomunicipales originales (`pob_turistica_equiv` y 4 métricas EOH de ocupación mensual) sin tocar `gold_municipio_master`/`gold_municipio_mensual` directamente.
- **`sentimiento_medio` agregado por hexágono ya no es una brecha** — corrige la brecha listada en la actualización de 02-sep-2026 (línea de "Brechas de datos identificadas"): el dato se resolvió en algún momento entre esa fecha y esta, y hoy es una de las 14 variables del modelo PTNA v3 y del ESG H3.
- **`gold.gold_aena_pasajeros` — confirmada su existencia** en Postgres. Corrige el "no" que figura para AENA en la columna Gold de la tabla de la sección 3 (ese dato quedó desactualizado).
...
- ✅ **Resuelto (16-sep-2026)**: `gold_h3_esg_v1` estaba en producción sin script versionado en el repo — formalizado en `analytics/mgwr/scripts/07_gold_h3_esg.py`, validado contra la tabla real.
- 🟡 **Nuevo (16-sep-2026) — Hallazgo 12, definitivo, no se revisará en una versión futura**: 9 de las 14 variables del modelo PTNA (`slope_mean`, `dist_hospital_km`, `pct_area_enp`, `temp_media_anual`, `lluvia_mm_anual`, `dist_parada_cercana_m`, `tiempo_aeropuerto_min`, `dist_costa_km`, `sentimiento_medio`) fueron tratadas únicamente como variables globales por el ajuste MGWR (bandwidth casi-máximo, 2573 de un techo de 2579), no como variables locales. Se agregó un tercer criterio de `confianza_ptna` para marcar los hexágonos afectados (259/2579 en total, sumando los 3 criterios). `gold_h3_ptna_v3` es la versión final que se entrega — esta limitación queda documentada como parte del resultado definitivo del modelo, no como pendiente de corrección.

## 7. Backlog general (detalle completo en el canvas, Zona 5)

- **Gold — Bloque 5 (MGWR/PTNA + ESG Territorial)**: asignado a jcabrera1605-netizen (02-sep-2026). **Subtareas 5.1, 5.2 y 5.3 cerradas** (16-sep-2026) — modelo PTNA v3 en producción (`gold_h3_ptna_v3`), Índice ESG cerrado en ambas escalas (`gold_h3_esg_v1` por hexágono, `gold_bloque5_municipio_esg_v1` por municipio), y filtro combinado PTNA+ESG construido (`gold_bloque5_h3_oportunidad_v1`). Subtarea 5.4 (elasticidades/análisis multiescala) descartada del alcance final del TFM. Documentación técnica detallada de todo el bloque en `contexto_maestro_proyecto_ptna.md`, siguiendo el mismo patrón que `contexto_maestro_proyecto.md` para Booking.
