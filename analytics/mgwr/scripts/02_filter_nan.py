...
Excluye hexagonos con mas del 50% de NULL entre las 9 variables X de
referencia (ver docs/contexto_maestro_proyecto_ptna.md, seccion 3). No toca
Postgres -- opera solo sobre el parquet local.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Filtra hexagonos con > 50% de NaN en las 9 variables X del Bloque 5")
    parser.add_argument(
        "--input",
        default=str(DATA_INTERIM_DIR / "ptna_dataset_raw.parquet"),
        help="Parquet de entrada (default: analytics/mgwr/data/interim/ptna_dataset_raw.parquet)",
    )
    parser.add_argument(
        "--output",
        default=str(DATA_INTERIM_DIR / "ptna_dataset_filtered.parquet"),
        help="Parquet de salida (default: analytics/mgwr/data/interim/ptna_dataset_filtered.parquet)",
    )
    parser.add_argument(
        "--max-nan-ratio",
        type=float,
...
def main():
    ...
    try:
        ...
        missing_quality_cols = [c for c in PTNA_QUALITY_COLUMNS if c not in df.columns]
        ...
        nan_ratio = df[PTNA_QUALITY_COLUMNS].isna().mean(axis=1)
        excluded_mask = nan_ratio > args.max_nan_ratio
        n_excluded = int(excluded_mask.sum())
        n_kept = len(df) - n_excluded

        logger.info(
            "Umbral de exclusion: > %.0f%% de NULL entre %d variables de calidad.",
            args.max_nan_ratio * 100, len(PTNA_QUALITY_COLUMNS),
        )
        logger.info("Filas excluidas: %d / %d", n_excluded, len(df))
        logger.info("Filas conservadas: %d", n_kept)

        excluded_df = df.loc[excluded_mask, PTNA_QUALITY_COLUMNS]
        ...
        logger.info(
            "NaNs por columna, dataset completo (antes de excluir):\n%s",
    ...
    print(f"Excluidas (> {args.max_nan_ratio:.0%} NaN en las 9 vars de calidad): {n_excluded}")
