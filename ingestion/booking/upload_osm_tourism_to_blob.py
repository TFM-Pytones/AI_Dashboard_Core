...
def process_and_upload():
    ...
    try:
        ...
        geojson_filename = "osm_pois_tenerife.geojson"
        geojson_path = os.path.join(ESPACIAL_RAW_DIR, geojson_filename)

        with open(geojson_path, "w", encoding="utf-8") as f:
            json.dump(geojson_data, f, ensure_ascii=False)
        
        logger.info(f"Guardado localmente en: {geojson_path}")

        # Subir a Azure
        blob_destination_path = f"{BLOB_FOLDER}/{geojson_filename}"
