...
def generate_h3_grid(resolution=8):
    ...
    # Esto "engorda" la silueta de Tenerife. Así nos aseguramos de que el centro
    # de los hexágonos costeros caiga siempre dentro, cubriendo el 100% de la isla real.
...
if __name__ == "__main__":
    out_dir = os.path.join("data", "bronce", "spatial", "h3")
    os.makedirs(out_dir, exist_ok=True)
    out_file = os.path.join(out_dir, "h3_grid_tenerife_res8.geojson")
    
    gdf_h3 = generate_h3_grid(resolution=8)
    
    print(f"Guardando localmente en {out_file}...")
    gdf_h3.to_file(out_file, driver="GeoJSON")
    
    blob_path = "spatial/h3/h3_grid_tenerife_res8.geojson"
